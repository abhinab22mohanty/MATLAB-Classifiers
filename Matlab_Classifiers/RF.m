...
function final = RF(edfFile1,edfFile2)
signal1 = FeatureExt(edfFile1);
signal2 = FeatureExt(edfFile2);
training_size = 60;

final = Comparator(signal1, signal2, training_size);

end    


function out = FeatureExt(edfFile)
[header, recorddata] = edfread(edfFile);
%data = output.getData([1 9760],1);
signal = transpose(recorddata);
% in seconds
%signal_len = 120;
signal_len = 61;
% sample rate in HZ
...
RF_input = [signature_data;signal_data];

%O1 = fitNaiveBayes(NBC_input,labels);
O1 = TreeBagger(100,RF_input,labels,'OOBPrediction','On','Method','classification');
C1 = predict(O1,RF_input);
%cMat = confusionmat(labels,C1)
cMat = confusionmat(labels,str2double(C1));
trueNegative=cMat(1,1);
falsePositive=cMat(1,2);
falseNegative=cMat(2,1);
truePositive=cMat(2,2);
